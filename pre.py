...
def preproc(pivot_num,pivot_min_st,src,dest):
   ...
   # stopwords =None
  #  bigram_vectorizer = CountVectorizer(ngram_range=(1, 2), token_pattern=r'\b\w+\b', min_df=5,binary=True, stop_words=stopwords)
    bigram_vectorizer = feature_extraction.text.TfidfVectorizer(ngram_range=(1, 2), token_pattern=r'\b\w+\b', stop_words=stopwords, binary=True, min_df=5)
    ...
    bigram_vectorizer_unlabeled = CountVectorizer(ngram_range=(1, 2), token_pattern=r'\b\w+\b', min_df=un_count, binary=True, stop_words=stopwords)
    #bigram_vectorizer_unlabeled = feature_extraction.text.TfidfVectorizer(ngram_range=(1, 2), token_pattern=r'\b\w+\b', stop_words=stopwords, binary=True, min_df=5)
    X_2_train_unlabeled = bigram_vectorizer_unlabeled.fit_transform(unlabeled).toarray()


    bigram_vectorizer_source = CountVectorizer(ngram_range=(1, 2), token_pattern=r'\b\w+\b', min_df=src_count, binary=True, stop_words=stopwords)
    #bigram_vectorizer_source = feature_extraction.text.TfidfVectorizer(ngram_range=(1, 2), token_pattern=r'\b\w+\b', stop_words=stopwords, binary=True, min_df=5)
    X_2_train_source = bigram_vectorizer_source .fit_transform(source).toarray()

    bigram_vectorizer_target = CountVectorizer(ngram_range=(1, 2), token_pattern=r'\b\w+\b', min_df=20, binary=True, stop_words=stopwords)
    #bigram_vectorizer_target = feature_extraction.text.TfidfVectorizer(ngram_range=(1, 2), token_pattern=r'\b\w+\b', stop_words=stopwords, binary=True, min_df=5)
